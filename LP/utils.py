...
class Unit():
    def __init__(self, id, cost, value=None, modifier=None, min=None, max=None):
        assert value is None or modifier is None, "Either value or modifier should be provided, not both."
        if value is not None:
            self.value = value
        elif modifier is not None:
            self.value = cost * modifier
        else:
            self.value = cost  # Default value equals cost
        self.id = id
        self.cost = cost
        self.min = min
        self.max = max
...
class Army():
    ...
    def add_noise(self, noise_factor=0.1):
        for unit in self.units:
            noise = unit.value * noise_factor * (2 * np.random.rand() - 1)  # Uniform noise in [-noise_factor*value, noise_factor*value]
            unit.value += noise
        self.init_solver()  # Re-initialize solver to update objective with new values
